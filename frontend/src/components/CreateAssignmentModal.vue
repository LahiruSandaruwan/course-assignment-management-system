      ...
      <form @submit.prevent="submit">
        <div class="form-group">
          ...
          <input
            ...
            placeholder="e.g. Build a Fullstack App"
          />
        ...
        <div class="form-row">
          ...
          <div class="form-group">
            <label for="assignment-max-score">Maximum Score</label>
            <input
              ...
              :disabled="isSubmitting"
            />
            <span v-if="errors.max_score" class="error-text">{{ errors.max_score[0] }}</span>
          </div>
        </div>

        <div class="form-group">
          <label for="assignment-status">Status</label>
          <select id="assignment-status" v-model="form.status" :disabled="isSubmitting">
...
const submit = async () => {
  ...
  try {
    const response = await assignmentService.createAssignment(props.courseId, { ...form });
    ...
    if (err.response?.status === 422) {
      ...
      generalError.value = err.response?.data?.message || 'Failed to create assignment.';
    }
  } finally {
